...
void rb_i32_del_test(RBTree *root, i32 v){
    if (root->root == root->NIL)return;
    RBNode *h = root->root;
    RBNode *pre = root->NIL;
    while (h != root->NIL){
    ...
    RBNode *rep = root->NIL;
    RBNode *del = h;
    if (del->left == root->NIL){
        rep = del->right;
    } else if (del->right == root->NIL){
        rep = del->left;
    } else{ // 两个都有
        RBNode *min = rb_i32_min(root , h->right);
        del->v = min->v;
...
void rb_i32_left_rotate_test(RBTree *root, RBNode *h){
    RBNode *x = h->right;
    h->right = x->left;
    if (x->left != root->NIL)
        x->left->parent = h;
...
void add_fixup_test(RBTree* root , RBNode* new){
    if (new->parent == root->NIL)return;
    while (new->parent != root->NIL && new->parent->color == RED){
        RBNode *parent = new->parent;
        if (parent == parent->parent->left){
            ...
            if (uncle->color == RED){
                ...
                rb_i32_right_rotate(root , grand);
                parent->color = BLACK;
